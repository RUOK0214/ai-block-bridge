...
현재 개발 버전: **0.2.2**. 중요한 월드의 복사본에서 먼저 테스트하세요.

## 설치

1. Fabric Loader **0.19.5 이상**, Minecraft **26.2**를 사용합니다.
2. `ai-block-bridge-0.2.2.jar`와 Fabric API **0.160.0+26.2 이상(26.2용)**을 `mods` 폴더에 넣습니다. 기존 버전 JAR는 제거합니다.
...
자동화가 필요할 때만 별도 애드온 ai-block-bridge-2-0.2.2.jar를 함께 설치하세요. 이 애드온은 기본 모드 0.2.2 이상이 필요합니다.
...
- 기록하지 않은 좌표는 기본적으로 **유지**합니다. `# unlisted: keep`을 명시해도 같습니다. 특정 좌표만 제거하려면 `minecraft:air`를 적으세요.
- 선택 영역 전체를 교체하려면 스크립트 앞부분에 `# unlisted: clear`를 한 줄 추가하세요. 적지 않은 좌표의 블록과 통 내용물이 삭제되며, 선택 영역 밖은 유지됩니다. 영역 범위는 파일 주석이 아니라 **현재 선택 영역**입니다.
- 구조 편집창에 현재 keep/clear 모드가 표시되며, clear 배치 확인 창에는 삭제 범위가 안내됩니다. 잘못된 값이나 중복 선언은 실행 전에 거부합니다. 지시자는 한 번만 선언하며 값은 소문자 `keep` 또는 `clear`입니다. 블록 행은 최소 한 개 필요합니다.
- 붙여넣기 취소는 삭제된 블록·통 내용물과 새로 배치된 위치의 원래 공기까지 복원합니다. 복원 데이터가 한도를 넘으면 변경 전에 거부합니다. 마지막 붙여넣기 한 번의 데이터만 메모리에 보관하며 접속 종료 시 사라집니다. 복구 파일을 디스크에 저장하지 않습니다.
- 자동화 애드온은 JSON `clear:true` 또는 스크립트 `# unlisted: clear` 중 하나라도 지정되면 비웁니다. JSON `clear` 기본값은 true이며 `keep`이 이를 취소하지 않습니다. 애드온 실행은 기본 모드의 붙여넣기 취소 기록을 만들지 않습니다. [0.2.2 변경사항](docs/releases/v0.2.2.md)
