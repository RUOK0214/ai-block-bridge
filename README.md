...
현재 버전: **0.1.3**. 중요한 월드의 복사본에서 먼저 테스트하세요.

## 설치

1. Fabric Loader **0.19.5 이상**, Minecraft **26.2**를 사용합니다.
2. `ai-block-bridge-0.1.3.jar`와 Fabric API **0.160.0+26.2 이상(26.2용)**을 `mods` 폴더에 넣습니다. 기존 버전 JAR는 제거합니다.
...
표시 격자는 작은 영역에서 1블록 간격이며, 긴 축은 최대 64분할에 맞춰 간격을 넓힙니다. 작업 한도 4,096블록을 넘는 선택은 빨간 외곽선으로 표시합니다. 표시 켜짐 상태는 클라이언트 세션 동안 유지합니다.
...
- 최대 1,200틱(약 60초) 또는 2,000,000자까지 기록합니다.
