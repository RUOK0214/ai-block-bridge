...
2. `ai-block-bridge-0.1.6.jar`와 Fabric API **0.160.0+26.2 이상(26.2용)**을 `mods` 폴더에 넣습니다. 기존 버전 JAR는 제거합니다.
...
서버 응답은 클라이언트에서 번역하므로 멀티플레이에서도 각자의 언어로 표시됩니다. **서버와 클라이언트를 모두 0.1.6으로 업데이트하세요.** 구조·타임라인의 블록 ID와 데이터 문법은 언어 설정에 관계없이 동일하며, 자동 생성되는 파일 주석은 영어입니다.
